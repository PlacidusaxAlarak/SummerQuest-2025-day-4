...
lora_rank = 16
lora_alpha = 32  # 缩放因子,通常是rank的两倍
lora_target_modules = [
    "q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"
]  # o_proj是输出投影层
num_train_epochs = 3
learning_rate = 2e-4
per_device_eval_batch_size = 2  # 评估阶段处理多少数据样本
gradient_accumulation_steps = 8  # 累计8个epoch后再计算梯度
seed = 42
max_length = 512
...
def main():
    ...
    data_collator = DataCollatorForSeq2Seq(
        tokenizer=tokenizer,
        model=model,  # 可选选项
        label_pad_token_id=-100,  # collator用-100来填充labels
        pad_to_multiple_of=8  # 将长度填充到8的倍数，可以提升训练速度
    )
